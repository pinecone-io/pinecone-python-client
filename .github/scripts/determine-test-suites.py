...
PATH_MAPPINGS = [
    ...
    # plugin mappings
    ("pinecone/deprecated_plugins.py", [
        "plugins",
    ]),
    ("pinecone/langchain_import_warnings.py", [
        "plugins",
    ]),
]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Determine test suites to run based on changed files")
    parser.add_argument(
        "--base-ref",
        default="main",
        help="Base branch/ref to compare against (default: main)",
    )
    parser.add_argument(
        "--run-all",
        action="store_true",
        help="Force running all test suites",
    )
    parser.add_argument(
        "--output-format",
