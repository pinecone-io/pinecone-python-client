...
class GRPCIndex(GRPCIndexBase):
    ...
    def query(self, queries, **kwargs):
        ...
        request = QueryRequest(
            queries=list(map(_query_transform, queries)), **{k: v for k, v in kwargs.items() if k in _QUERY_ARGS}
        )
...
class CIndex(GRPCIndex):
    ...
    def upsert(
        self, request: "vector_column_service_pb2.UpsertRequest", timeout: int = None, metadata: Dict[str, str] = None
    ):
        return self._wrap_grpc_call(self.stub.Upsert, request, timeout=timeout, metadata=metadata)

    def delete(
        self, request: "vector_column_service_pb2.DeleteRequest", timeout: int = None, metadata: Dict[str, str] = None
    ):
        return self._wrap_grpc_call(self.stub.Delete, request, timeout=timeout, metadata=metadata)

    def fetch(
        self, request: "vector_column_service_pb2.FetchRequest", timeout: int = None, metadata: Dict[str, str] = None
    ):
        return self._wrap_grpc_call(self.stub.Fetch, request, timeout=timeout, metadata=metadata)

    def query(
        self, request: "vector_column_service_pb2.QueryRequest", timeout: int = None, metadata: Dict[str, str] = None
    ):
