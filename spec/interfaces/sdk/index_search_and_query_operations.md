...
### `Index.search(namespace: str, query: SearchQueryTypedDict | SearchQuery, rerank: SearchRerankTypedDict | SearchRerank | None = None, fields: list[str] | None = ["*"]) -> SearchRecordsResponse`
...
**Import:** `from pinecone import Pinecone, SearchQuery, SearchRerank, RerankModel`
**Source:** `pinecone/db_data/index.py:660-744`
**Added:** v8.1
**Deprecated:** No
**Idempotency:** Safe to retry
**Side effects:** Queries the index; no mutations
**Async variant:** `async def search(...)` in `pinecone/db_data/index_asyncio.py:1400-1501`

**Parameters**

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| namespace | str | Yes | — | The namespace in the index to search. |
| query | SearchQueryTypedDict \| SearchQuery | Yes | — | The search query object containing `inputs` (dict) and `top_k` (int). May optionally include `filter`, `vector`, `id`, or `match_terms`. See **SearchQuery Details** below. |
| rerank | SearchRerankTypedDict \| SearchRerank \| None | No | None | Optional reranking configuration to apply to search results. Specifies the reranking model, fields to rank against, and optionally the number of top results to return. See **SearchRerank Details** below. |
| fields | list[str] \| None | No | ["*"] | List of fields to return in the response. Use `["*"]` to return all fields. |
...
**Returns:** `SearchRecordsResponse` — A response object containing:
...
### `Index.search_records(namespace: str, query: SearchQueryTypedDict | SearchQuery, rerank: SearchRerankTypedDict | SearchRerank | None = None, fields: list[str] | None = ["*"]) -> SearchRecordsResponse`

Alias of `search()`. Provides the same functionality with an alternative method name.

**Import:** `from pinecone import Pinecone, SearchQuery, SearchRerank`
**Source:** `pinecone/db_data/index.py:746-759`
**Added:** v8.1
**Deprecated:** No
**Idempotency:** Safe to retry
**Side effects:** Queries the index; no mutations
**Async variant:** `async def search_records(...)` in `pinecone/db_data/index_asyncio.py:1503-1515`

See `Index.search()` for complete documentation.

---

### `Index.query(top_k: int, vector: list[float] | None = None, id: str | None = None, namespace: str | None = None, filter: FilterTypedDict | None = None, include_values: bool | None = None, include_metadata: bool | None = None, sparse_vector: SparseValues | SparseVectorTypedDict | None = None, scan_factor: float | None = None, max_candidates: int | None = None, **kwargs) -> QueryResponse | ApplyResult`
...
**Import:** `from pinecone import Pinecone, SparseValues`
**Source:** `pinecone/db_data/index.py:936-1056`
**Added:** v8.0
**Deprecated:** No
**Idempotency:** Safe to retry
**Side effects:** Queries the index; no mutations
**Async variant:** `async def query(...)` in `pinecone/db_data/index_asyncio.py:720-905`

**Parameters**

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| top_k | int | Yes | — | The number of results to return for each query. Must be an integer greater than or equal to 1. |
| vector | list[float] \| None | No | None | The query vector (dense embedding). Must be the same length as the index dimension. Cannot be used together with `id`. |
| id | str \| None | No | None | The unique ID of an existing vector to use as the query vector. Cannot be used together with `vector`. |
| namespace | str \| None | No | None | The namespace to query. If omitted, the default (empty string) namespace is used. |
| filter | FilterTypedDict \| None | No | None | Optional metadata filter to apply. Limits results to vectors whose metadata matches the filter. See **Filter Details** below. |
| include_values | bool \| None | No | None | Whether vector values should be included in the response. Default behavior (None) means values are not included. |
| include_metadata | bool \| None | No | None | Whether metadata should be included in the response. Default behavior (None) means metadata is not included. |
| sparse_vector | SparseValues \| SparseVectorTypedDict \| None | No | None | Optional sparse vector for hybrid search. Expected format: `{"indices": list[int], "values": list[float]}` or a `SparseValues` object. |
| scan_factor | float \| None | No | None | Optimization parameter for IVF dense indexes in dedicated read node (DRN) indexes. Adjusts how much of the index is scanned (range: 0.5-4, default 1.0). Only supported for DRN dense indexes. |
| max_candidates | int \| None | No | None | Optimization parameter for DRN dense indexes. Controls the maximum number of candidate dense vectors to rerank (range: top_k to 100,000). Only supported for DRN dense indexes. |
| **kwargs | dict | No | — | Additional keyword arguments for the API call. Supported kwargs: `async_req` (bool) — if True, returns `ApplyResult` instead of `QueryResponse`; `async_threadpool_executor` (bool) — if True, uses thread pool for async execution. |
...
**Returns:**
- `QueryResponse` (default) — A response object containing:
...
- `ApplyResult` (if `async_req=True` or `async_threadpool_executor=True`) — An async result wrapper. Call `.get()` to retrieve the actual `QueryResponse` (may block).
...
### `AsyncIndex.search(namespace: str, query: SearchQueryTypedDict | SearchQuery, rerank: SearchRerankTypedDict | SearchRerank | None = None, fields: list[str] | None = ["*"]) -> Coroutine[Any, Any, SearchRecordsResponse]`
...
### `AsyncIndex.search_records(namespace: str, query: SearchQueryTypedDict | SearchQuery, rerank: SearchRerankTypedDict | SearchRerank | None = None, fields: list[str] | None = ["*"]) -> Coroutine[Any, Any, SearchRecordsResponse]`
...
### `AsyncIndex.query(top_k: int, vector: list[float] | None = None, id: str | None = None, namespace: str | None = None, filter: FilterTypedDict | None = None, include_values: bool | None = None, include_metadata: bool | None = None, sparse_vector: SparseValues | SparseVectorTypedDict | None = None, scan_factor: float | None = None, max_candidates: int | None = None, **kwargs) -> Coroutine[Any, Any, QueryResponse]`
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| inputs | dict[str, Any] | Yes | Input data for the embedding model. |
| top_k | int | Yes | Number of results to return. |
| filter | dict[str, Any] \| None | No | Optional metadata filter. |
| vector | SearchQueryVectorTypedDict \| SearchQueryVector \| None | No | Optional vector to search with (overrides inputs). |
| id | str \| None | No | Optional vector ID to search with (overrides inputs). |
| match_terms | dict[str, Any] \| None | No | Optional matching strategy and terms for keyword matching. |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| model | str \| RerankModel | Yes | Name of the reranking model to use. |
| rank_fields | list[str] | Yes | Fields to use for reranking. |
| top_n | int \| None | No | Number of top results after reranking (defaults to top_k). |
| parameters | dict[str, Any] \| None | No | Additional model-specific parameters. |
| query | str \| None | No | Custom query for reranking (overrides top-level query). |
...
|-------|------|-------------|
| matches | list[ScoredVector] | List of matched vectors with scores. Each vector contains: `id` (str), `score` (float), `values` (list[float] \| None), `metadata` (dict \| None). |
| namespace | str | The namespace that was queried. |
| usage | Usage \| None | Usage statistics (optional). |
| _response_info | ResponseInfo | Response metadata including headers (internal use). |
...
| Field | Type | Description |
|-------|------|-------------|
| indices | list[int] | List of non-zero indices in the sparse vector. |
| values | list[float] | List of non-zero values corresponding to the indices. |
...
### `SearchQueryTypedDict` (TypedDict)
...
### `SearchRerankTypedDict` (TypedDict)
...
### `SparseVectorTypedDict` (TypedDict)
