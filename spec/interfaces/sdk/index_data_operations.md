...
### `Index.upsert(vectors: list[Vector] | list[VectorTuple] | list[VectorTupleWithMetadata] | list[VectorTypedDict], namespace: str | None = None, batch_size: int | None = None, show_progress: bool = True, **kwargs) -> UpsertResponse | ApplyResult`
...
**Import:** `from pinecone import Pinecone, Vector`
**Source:** `pinecone/db_data/index.py:226-442`
**Added:** v8.0
**Deprecated:** No
...
**Async variant:** `async def upsert(...)` in `pinecone/db_data/index_asyncio.py:292-467`

**Parameters**

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| vectors | list[Vector] \| list[VectorTuple] \| list[VectorTupleWithMetadata] \| list[VectorTypedDict] | Yes | — | List of vectors to upsert. Each vector must have a unique `id` within the namespace. Vectors can be provided as: `Vector` objects, 2-tuples `(id, values)` for dense vectors without metadata, 3-tuples `(id, values, metadata)` for dense vectors with metadata, or dictionaries with `id`, `values`, and optional `metadata` and `sparse_values` keys. |
| namespace | str | No | `""` (default namespace) | The namespace to write vectors to. If not specified, vectors are upserted into the default empty-string namespace. |
| batch_size | int | No | None | Number of vectors to upsert in each batch. If not specified, all vectors are upserted in a single request. When specified, vectors are sent in batches with the `tqdm` progress bar shown if `show_progress=True`. Cannot be used with `async_req=True`. |
| show_progress | bool | No | True | Whether to display a progress bar using `tqdm` when upserting in batches. Only applied when `batch_size` is provided. Requires `tqdm` to be installed. |
| **kwargs | dict | No | — | Additional keyword arguments, including `async_req` (bool) to execute the request asynchronously. |
...
**Returns:** `UpsertResponse` — A response object containing:
...
### `Index.delete(ids: list[str] | None = None, delete_all: bool | None = None, namespace: str | None = None, filter: FilterTypedDict | None = None, **kwargs) -> dict[str, Any]`
...
**Source:** `pinecone/db_data/index.py:762-824`
**Added:** v8.0
**Deprecated:** No
...
**Async variant:** `async def delete(...)` in `pinecone/db_data/index_asyncio.py:512-598`

**Parameters**

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| ids | list[str] | No | None | List of vector IDs to delete from the namespace. Mutually exclusive with `delete_all` and `filter`. If none of the three parameters are specified, the method raises an error. |
| delete_all | bool | No | None | If `True`, deletes all vectors from the namespace. Mutually exclusive with `ids` and `filter`. Must be explicitly set to `True`; the default is `None` (not `False`). |
| namespace | str | No | `""` (default namespace) | The namespace to delete vectors from. If not specified, the default empty-string namespace is used. **Note:** No error is raised if the namespace does not exist. |
| filter | FilterTypedDict | No | None | Metadata filter expression to select vectors for deletion. Vectors matching the filter are deleted from the namespace. Mutually exclusive with `ids` and `delete_all`. See metadata filtering documentation for filter syntax. |
| **kwargs | dict | No | — | Additional keyword arguments for the API call. |
...
**Returns:** `dict[str, Any]` — An empty dictionary `{}` on success. No metadata about deleted vector count is returned.
