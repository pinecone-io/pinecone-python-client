...
from tests.integration.helpers import fake_api_key, embedding_values
...
class TestUpsertFailsWhenDimensionMismatch:
    ...
    def test_upsert_fails_when_dimension_mismatch_tuples(self, idx):
        with pytest.raises(PineconeException):
            idx.upsert(vectors=[("1", embedding_values(2)), ("2", embedding_values(3))])
...
class TestUpsertFailsSparseValuesDimensionMismatch:
    def test_upsert_fails_when_sparse_values_indices_values_mismatch_objects(self, hybrid_idx):
        with pytest.raises(PineconeException):
            hybrid_idx.upsert(
                vectors=[
                    Vector(
    ...
    def test_upsert_fails_when_sparse_values_in_tuples(self, hybrid_idx):
    ...
    def test_upsert_fails_when_sparse_values_indices_values_mismatch_dicts(self, hybrid_idx):
        with pytest.raises(PineconeException):
            hybrid_idx.upsert(
                vectors=[
                    {
