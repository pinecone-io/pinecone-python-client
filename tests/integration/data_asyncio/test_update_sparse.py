import pytest
import asyncio
from pinecone import Vector, SparseValues, PineconeApiException, PineconeApiTypeError
...
class TestAsyncioUpdateSparse:
    async def test_update_values(self, pc, sparse_index_host, target_namespace):
        ...
        await asyncio_idx.update(
            id="1",
            sparse_values=new_sparse_values,
            namespace=target_namespace,
        )
        
        async def wait_condition():
            fetched_vec = await asyncio_idx.fetch(ids=["1"], namespace=target_namespace)
            return fetched_vec.vectors['1'].sparse_values.values[0] == pytest.approx(new_sparse_values['values'][0], 0.01)
        ...
        for i in range(100):
            assert fetched_vec.vectors['1'].sparse_values.values[i] == pytest.approx(new_sparse_values['values'][i], 0.01)
