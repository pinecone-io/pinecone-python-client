...
async def poll_for_freshness(asyncio_idx, target_namespace, target_vector_count):
    ...
    while True:
        ...
        logger.debug("Polling for freshness on index %s. Current vector count: %s. Waiting for: %s", asyncio_idx, stats.total_vector_count, target_vector_count)
        if target_namespace == "":
        ...
        if time_waited >= max_wait_time:
            raise TimeoutError("Timeout waiting for index to have expected vector count of {}".format(target_vector_count))
...
async def wait_until(
    ...
    while True:
        ...
        logger.debug("Condition not met yet. Waiting for %.2f seconds. Timeout in %.2f seconds.", interval, (start_time + timeout) - asyncio.get_event_loop().time())
