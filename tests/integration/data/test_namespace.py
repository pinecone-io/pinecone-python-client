...
class TestNamespaceOperations:
    ...
    def test_delete_namespace(self, idx):
        ...
        # Wait for namespace to be deleted with polling
        max_wait_time = 60  # 60 seconds max wait
        poll_interval = 2  # Check every 2 seconds
        total_wait = 0

        while total_wait < max_wait_time:
            if not verify_namespace_exists(idx, test_namespace):
                break
            time.sleep(poll_interval)
            total_wait += poll_interval

        # Verify namespace is deleted
        assert not verify_namespace_exists(
            idx, test_namespace
        ), f"Namespace {test_namespace} still exists after {total_wait} seconds"
