...
def test_multiple_indexes():
    ...
    responses.add(
        "GET",
        f"/describe_index_stats",
        status=200,
        content_type="application/json",
        adding_headers={"date": "Thu, 28 Oct 2021 09:14:51 GMT", "server": "envoy"},
...
def test_invalid_delete_response_unrecognized_field():
    ...
    responses.add(
        "DELETE", "/vectors/delete", body='{"unexpected_key": "xyzzy"}', status=200, content_type="application/json"
    )
...
def test_delete_response_missing_field():
    ...
    pinecone.init("example-api-key", environment="example-environment")
    responses.add("DELETE", "/vectors/delete", body="{}", status=200, content_type="application/json")
    index = pinecone.Index("example-index")
...
def _test_invalid_delete_response_wrong_type():
    ...
    responses.add(
        "DELETE", "/vectors/delete", body='{"deleted_count": "foobar"}', status=200, content_type="application/json"
    )
