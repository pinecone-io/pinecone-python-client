...
class TestPineconeIndexDelegate:
    def test_forwards(self) -> None:
        ...
        pc = Pinecone(api_key="test-key")
        with patch.object(pc, "_resolve_index_host", return_value="my-index.svc.pinecone.io"):
            idx = pc.Index(name="x", host="h")
    ...
    def test_forwards_pool_threads(self) -> None:
        ...
        pc = Pinecone(api_key="test-key")
        with patch.object(pc, "_resolve_index_host", return_value="my-index.svc.pinecone.io"):
            idx = pc.Index(name="x", host="h", pool_threads=20)  # type: ignore[call-arg]
        assert isinstance(idx, Index)
