...
class TestGrpcIndex:
    def setup_method(self):
        ...
        self.expected_vec_md_sparse1 = Vector(
            id="vec1",
            values=self.vals1,
            metadata=dict_to_proto_struct(self.md1),
            sparse_values=SparseValues(indices=self.sparse_indices_1, values=self.sparse_values_1),
        )
        self.expected_vec_md_sparse2 = Vector(
            id="vec2",
            values=self.vals2,
            metadata=dict_to_proto_struct(self.md2),
    ...
    def test_upsert_tuplesOfIdVec_UpserWithoutMD(self, mocker):
        ...
        self._assert_called_once([self.expected_vec1, self.expected_vec2])
    ...
    def test_upsert_tuplesOfIdVecMD_UpsertVectorsWithMD(self, mocker):
        ...
        self._assert_called_once(
            [self.expected_vec_md1, self.expected_vec_md2],
        )

    def test_upsert_vectors_upsertInputVectors(self, mocker):
        mocker.patch.object(self.index, "_wrap_grpc_call", autospec=True)
        self.index.upsert([self.expected_vec_md1, self.expected_vec_md2], namespace="ns")
        self._assert_called_once(
            [self.expected_vec_md1, self.expected_vec_md2],
        )
    ...
    def test_upsert_dict_md(self, mocker):
        ...
        self.index.upsert([dict1, dict2], namespace="ns")
        self._assert_called_once([self.expected_vec_md1, self.expected_vec_md2])
    ...
    def test_upsert_dataframe(self, mocker):
        ...
        self._assert_called_once([self.expected_vec_md_sparse1, self.expected_vec_md_sparse2], async_call=True)
    ...
    def test_upsert_dataframe_sync(self, mocker):
        ...
        self._assert_called_once([self.expected_vec_md_sparse1, self.expected_vec_md_sparse2], async_call=False)
    ...
    def test_upsert_async_upsertInputVectorsAsync(self, mocker):
        mocker.patch.object(self.index, "_wrap_grpc_call", autospec=True)
        self.index.upsert([self.expected_vec_md1, self.expected_vec_md2], namespace="ns", async_req=True)
        self._assert_called_once([self.expected_vec_md1, self.expected_vec_md2], async_call=True)
    ...
    def test_upsert_vectorListIsMultiplyOfBatchSize_vectorsUpsertedInBatches(self, mocker):
        ...
        result = self.index.upsert(
            [self.expected_vec_md1, self.expected_vec_md2], namespace="ns", batch_size=1, show_progress=False
        )
        self.index._wrap_grpc_call.assert_any_call(
            self.index.stub.Upsert,
            UpsertRequest(
                vectors=[Vector(id="vec1", values=self.vals1, metadata=dict_to_proto_struct(self.md1))], namespace="ns"
            ),
            timeout=None,
        )

        self.index._wrap_grpc_call.assert_any_call(
            self.index.stub.Upsert, UpsertRequest(vectors=[self.expected_vec_md2], namespace="ns"), timeout=None
        )
    ...
    def test_upsert_vectorListNotMultiplyOfBatchSize_vectorsUpsertedInBatches(self, mocker):
        ...
        self.index._wrap_grpc_call.assert_any_call(
            self.index.stub.Upsert,
            UpsertRequest(vectors=[self.expected_vec_md1, self.expected_vec_md2], namespace="ns"),
            timeout=None,
        )
        ...
        self.index._wrap_grpc_call.assert_any_call(
            self.index.stub.Upsert,
            UpsertRequest(
                vectors=[Vector(id="vec3", values=self.vals1, metadata=dict_to_proto_struct(self.md1))], namespace="ns"
            ),
            timeout=None,
        )
    ...
    def test_upsert_vectorListSmallerThanBatchSize_vectorsUpsertedInBatches(self, mocker):
        ...
        result = self.index.upsert([self.expected_vec_md1, self.expected_vec_md2], namespace="ns", batch_size=5)
        self._assert_called_once(
            [self.expected_vec_md1, self.expected_vec_md2],
        )
    ...
    def test_upsert_tuplesList_vectorsUpsertedInBatches(self, mocker):
        ...
        result = self.index.upsert(
            [("vec1", self.vals1, self.md1), ("vec2", self.vals2, self.md2), ("vec3", self.vals1, self.md1)],
            namespace="ns",
            batch_size=2,
        )
        self.index._wrap_grpc_call.assert_any_call(
    ...
    def test_query_byTuplesNoFilter_queryVectorsNoFilter(self, mocker):
        ...
        self.index._wrap_grpc_call.assert_called_once_with(
            self.index.stub.Query,
            QueryRequest(
                queries=[QueryVector(values=self.vals1, filter={}), QueryVector(values=self.vals2, filter={})], top_k=10
            ),
            timeout=None,
        )
