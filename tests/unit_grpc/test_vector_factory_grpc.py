...
class TestVectorFactory:
    ...
    def test_build_when_nongrpc_vector_with_metadata(self):
        ...
        assert VectorFactoryGRPC.build(vec) == Vector(id='1', values=[0.1, 0.2, 0.3], metadata=dict_to_proto_struct({'genre': 'comedy'}))

    def test_build_when_nongrpc_vector_with_sparse_values(self):
        vec = NonGRPCVector(id='1', values=[0.1, 0.2, 0.3], sparse_values=NonGRPCSparseValues(indices=[0, 2], values=[0.1, 0.3]))
        assert VectorFactoryGRPC.build(vec) == Vector(id='1', values=[0.1, 0.2, 0.3], metadata=dict_to_proto_struct({}), sparse_values=SparseValues(indices=[0, 2], values=[0.1, 0.3]))
    ...
    def test_build_vector_with_tuple_with_sparse_values(self, sv_klass):
        ...
        with pytest.raises(ValueError, match="Sparse values are not supported in tuples. Please use either dicts or Vector objects as inputs."):
    ...
    def test_build_when_dict_with_sparse_values_dict(self):
        ...
        actual = VectorFactoryGRPC.build(d)
        expected = Vector(id='1', values=[0.1, 0.2, 0.3], metadata=dict_to_proto_struct({'genre': 'comedy'}), sparse_values=SparseValues(indices=[0, 2], values=[0.1, 0.3]))
        assert actual == expected

    @pytest.mark.parametrize('sv_klass', [SparseValues, NonGRPCSparseValues])
    def test_build_with_dict_with_sparse_values_object(self, sv_klass):
        d = { 'id': '1', 'values': [0.1, 0.2, 0.3], 'metadata': {'genre': 'comedy'}, 'sparse_values': sv_klass(indices=[0, 2], values=[0.1, 0.3])}
        actual = VectorFactoryGRPC.build(d)
        expected = Vector(id='1', values=[0.1, 0.2, 0.3], metadata=dict_to_proto_struct({'genre': 'comedy'}), sparse_values=SparseValues(indices=[0, 2], values=[0.1, 0.3]))
        assert actual == expected
